...
async def add_new_product_to_catalog(
    product: InsertProduct, token=Depends(check_if_is_admin_user)
):  # , user = Depends(get_current_user)
    product_id = add_new_product(product.model_dump())
    if product_id:
        return {"product_id": product_id, "status": "created successfully"}
    else:
...
async def update_catalog_product(
    product: UpdateProduct, id: str, token=Depends(check_if_is_admin_user)
):
    response = update_product(
        product=product.model_dump(exclude_unset=True), product_id=id
    )
    if response == True:
...
async def get_product_from_catalog_by_id(
    id, token=Depends(checking_basic_user_permissions)
):
...
async def get_all_product_from_catalog():
    try:
        ...
        products_hits = product_info["hits"]["hits"]  # type: ignore
